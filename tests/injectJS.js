...
browser.addCommand("isVideoPaused", function () {
    var isPaused = browser.execute(function () {
        console.log('outputted inside console window');
...
describe('Video test', function () {
    it('Validate that the video is paused on accessing the page', function(done){
        this.timeout(20000);
        browser.pause(6000);
    ...
    it('Alter the width of the video', function(done){
        var videoWidth = browser.execute(function(){
            var video = document.querySelector('#video1');
            return video.getElementsByClassName.width = '300px'
        });
        browser.pause(6000);
    });
});

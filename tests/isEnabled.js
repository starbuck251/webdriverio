...
describe("Test Enabled Dropdown Menus, Checkboxes & Radio buttons", function () {
    it('Dropdown item orange is disabled therefore return false', function (done) {
        var isEnabled = browser.isEnabled("option[value='orange']");
        console.log(isEnabled);
        expect(isEnabled).to.equal(false);
    });   
    
    it('Dropdown item grape is enabled therefore return true', function (done) {
        var isEnabled = browser.isEnabled("option[value='grape']");
        console.log(isEnabled);
        expect(isEnabled).to.equal(true);
    });    

    it('Option 2 is enabled therefore should return true', function (done) {
